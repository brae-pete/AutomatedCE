...
class PLTMicroscopeDisplay(MicroscopeDisplayAbstraction):
    ...
    def live_image(self):
        ...
        if self._ani_func is None:
            self._ani_func = animation.FuncAnimation(self.fig, self._animate, interval=100, blit=True)
...
class TkCEDisplay(CEDisplayAbstraction):

    def __init__(self, system: CESystem, parent_tk):
        super().__init__(system)
        self.fig = None
        self.plot_axes = []
        self._ani_func = None
        self._artists = []
        self.top = tkinter.Toplevel(parent_tk)
    ...
    def start_live_view(self):
        ...
        :return:
        """
        #self.parent.after(1000, self._animate)
        if self._ani_func is None:
            self._ani_func = animation.FuncAnimation(self.fig, self._animate, interval=500, blit=False)
